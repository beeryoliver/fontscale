...
Version 4.2.0\quad{}30 April 2025
...
If you get a warning that Computer Modern is not available in the requested font size, you may need to add the following code before \cs{documentclass} to make Computer Modern available at arbitrary font sizes:
\begin{verbatim}
...
If the user would like to add this functionality or perhaps other features to the font size commands, then they can use hooks, which are documented in \pkg{lthooks} and \pkg{ltcmdhooks}.
...
This syntax has the same representation as the arguments to \cs{fpeval}, \cs{inteval}, \cs{dimeval}, and \cs{skipeval}, respectively, documented in \pkg{usrguide}.
...
\meta{choice} must be any of the following: \keyvalue{classic-10pt}, \keyvalue{classic-11pt}, \keyvalue{classic-12pt}, \keyvalue{classic-10bp}, \keyvalue{classic-11bp}, \keyvalue{classic-12bp}, \keyvalue{classic-10dd}, \keyvalue{classic-11dd}, \keyvalue{classic-12dd}, \keyvalue{classic-10nd}, \keyvalue{classic-11nd}, \keyvalue{classic-12nd}, \keyvalue{musical}.
...
The named font size is the font size of \cs{normalsize}.
The font sizes of the other font size commands are the adjacent font sizes in the classic typographic scale in the named unit of measurement.
...
Issues a warning when overwriting the font size set by the latter key.
These keys are initially not set.
...
Issues a warning when overwriting the font baselineskip set by the latter key.
These keys are initially not set.
...
\begin{displaycode}
  \displaykey{magscale}[\meta{floating point expression}][\meta{not set}]
\end{displaycode}

Scales the font size and font baselineskip of each font size command from \cs{tiny} to \cs{Huge} by a factor equal to the result of computing the \meta{floating point expression}.
The new font baselineskips have no stretch and shrink components.
This key is applied after all the other keys.
...
Adding the optional star \sarg{} first resets all the keys to their initial values.
...
Expands to the current font step, font scale, font size, and font baselineskip, respectively.
These commands do not expand the kernel command \cs{f@size}.
...
If not and if the value of the key \key{typographic-scale} is \keyvalue{musical}, then \cs{currentfontstep} expands to the font step calculated from the musical typographic scale (Equation \ref{eq:musical}).
...
Scales the font size and font baselineskip by a factor equal to the result of computing the \meta{floating point expression}.
...
Sets the font size and font baselineskip to the value of \meta{dimen expression} and \meta{skip expression}, respectively.
Does nothing, except issue a warning, if used in math mode.
...
Stores the current font step, font scale, font size, and font baselineskip, respectively.
